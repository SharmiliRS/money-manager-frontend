...
const IncomeModal = ({ isOpen, onClose, onAddIncome, editingIncome }) => {
  ...
  useEffect(() => {
    if (editingIncome) {
      ...
      setIncomeData({
        email:
          editingIncome.userEmail || localStorage.getItem("userEmail") || "",
  ...
  const fetchCategories = useCallback(async (email) => {
    try {
      const response = await axios.get(
        `${BASE_URL}/categories/${email}?type=Income`,
      );
    ...
    try {
      ...
      if (editingIncome) {
        ...
        response = await axios.put(
          `${BASE_URL}/income/${editingIncome._id}`,
          payload,
          {
            headers: {
      ...
      if (error.response) {
        ...
        setError(
          `Error: ${error.response.data.error || error.response.data.message || "Failed to save income"}`,
        );
      } else if (error.request) {
        // Request made but no response
        setError(
          "No response from server. Please check if backend is running on port 5000.",
        );
      } else {
    ...
    <Modal show={isOpen} onHide={handleClose} centered size="lg">
          ...
          <div className="flex-grow-1">
            ...
            <p className="mb-0 opacity-90" style={{ fontSize: "0.9rem" }}>
              {editingIncome
                ...
                : "Record your income with all details"}
            </p>
          </div>
        </div>
      </Modal.Header>
      ...
      {editingIncome && (
        <div className="mt-3 px-3">
          <div
            className="alert alert-warning d-flex align-items-start mx-3"
            role="alert"
            style={{ borderRadius: "0.75rem" }}
          >
            <AlertCircle size={20} className="me-2 flex-shrink-0 mt-1" />
            <div>
              <strong>Editing Mode:</strong> You can only edit this income
