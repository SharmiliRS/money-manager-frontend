...
const Dashboard = () => {
  ...
  const fetchRecentTransactions = async () => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/transactions/recent/${userEmail}?limit=10`);
  ...
  const handleAddIncome = async (newIncome) => {
    try {
      await axios.post(`${BASE_URL}/income/add`, newIncome);
  ...
  const handleAddExpense = async (newExpense) => {
    try {
      await axios.post(`${BASE_URL}/expense/minus`, newExpense);
      await Promise.all([fetchDashboardData(), fetchRecentTransactions()]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <main className={`transition-all duration-300 pt-4 ${
        ...
        <div className={`px-4 lg:px-6 transition-all duration-300 ${
          ...
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-2 lg:mt-0 mt-1 text-center lg:text-left">
              Dashboard
            </h1>
          ...
          {/* Stats Cards */}
          {/* ... rest of your stats cards code remains the same ... */}
           {/* Stats Cards */}
                    {loading ? (
...
>
  ...
  <optgroup label="─── Expense Categories ───">
    ...
    <option value="Other Expense">💰 Other Expense</option>
          ...
          {/* Recent Transactions Section - Updated with enhanced table */}
          <div className="bg-white rounded-2xl shadow-lg p-6 mb-8 border border-gray-200">
                          ...
                          <div className="flex items-center gap-3">
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                              ...
                              {transaction.type === "income" ? (
                                <Plus size={16} className={transaction.type === "income" ? "text-emerald-600" : "text-red-600"} />
                              ) : (
                                <Minus size={16} className={transaction.type === "income" ? "text-emerald-600" : "text-red-600"} />
                              )}
                            </div>
                            <div>
